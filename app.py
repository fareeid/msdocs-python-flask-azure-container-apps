...
if "RUNNING_IN_PRODUCTION" not in os.environ:
    ...
    # Production, we don't load environment variables from .env file but add them as environment variables in Azure.  # noqa: E501
...
from models import Restaurant, Review  # noqa: E402, F401
...
def index():
    from models import Restaurant  # noqa: F811
...
def details(id, message):
    ...
    return render_template(
        "details.html", restaurant=restaurant, reviews=reviews, message=message
    )
...
def add_review(id):
    ...
    try:
        ...
        reviews = Review.query.where(Review.restaurant == id)
        return details(
            id, "Review not added. Include at least a name and rating for review."
        )
    else:
        review = Review()
